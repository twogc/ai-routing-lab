...
from models.prediction import JitterPredictor, LatencyPredictor, RoutePredictionEnsemble
...
def run_complete_experiment():
    ...
    X_latency_test_proc, y_latency_test_proc, _ = lab.prepare_data(X_latency_test, y_latency_test)
    X_jitter_test_proc, y_jitter_test_proc, _ = lab.prepare_data(X_jitter_test, y_jitter_test)
    ...
    latency_predictor = LatencyPredictor(n_estimators=100, max_depth=15, random_state=42)
    latency_predictor.fit(X_latency_train_proc, y_latency_train_proc)

    jitter_predictor = JitterPredictor(n_estimators=100, max_depth=15, random_state=42)
    ...
    print(
        f"   Latency accuracy: {latency_accuracy:.4f} ({'>92%' if latency_accuracy > 0.92 else '<92%'})"
    )
    print(
        f"   Jitter accuracy: {jitter_accuracy:.4f} ({'>92%' if jitter_accuracy > 0.92 else '<92%'})"
    )
