...
class LaboratoryExperiment:
    ...
    def __init__(
        ...
        self.model_registry = ModelRegistry(models_dir=str(self.models_dir), logger=self.logger)
    ...
    def prepare_data(
        self, X: np.ndarray, y: Optional[np.ndarray] = None, timestamps: Optional[np.ndarray] = None
    ...
    def train_model(self, X: np.ndarray, y: np.ndarray, model: Any) -> Tuple[Any, Dict[str, Any]]:
        """
    ...
    def save_experiment_results(
        ...
        result = ExperimentResult(
            ...
            artifacts={
                ...
                "config_file": str(
                    self.experiments_dir / f"{self.current_experiment.experiment_id}_config.json"
                ),
            },
        )
        ...
        # Save results
        results_file = (
            self.experiments_dir / f"{self.current_experiment.experiment_id}_results.json"
        )
...
def create_latency_prediction_experiment(
    hyperparameters: Optional[Dict[str, Any]] = None, data_config: Optional[Dict[str, Any]] = None
    ...
    lab.create_experiment(
        ...
        hyperparameters=hyperparameters
        or {"n_estimators": 100, "max_depth": 10, "random_state": 42},
        data_config=data_config
        or {
