...
class RandomForestRouteClassifier:
    ...
    def __init__(self, n_routes: int = 5, n_trees: int = 50, logger: Optional[logging.Logger] = None):
    ...
    def _predict_sample(self, x: np.ndarray, tree: Dict) -> int:
        if tree['leaf']:
            return tree['class']
        return self._predict_sample(x, tree['left' if x[tree['feature']] < tree['threshold'] else 'right'])
