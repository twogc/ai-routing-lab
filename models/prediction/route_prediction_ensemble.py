...
class RoutePredictionEnsemble:
    ...
    def predict_route(
        ...
        overall_confidence = (
            latency_pred.confidence_score + jitter_pred.confidence_score
        ) / 2.0
    ...
    def select_best_route(
        self,
        routes_features: Dict[int, Tuple[np.ndarray, np.ndarray]]
        ...
        best_route_id = max(route_predictions.keys(), key=lambda r: route_predictions[r].combined_score)
