...
class JitterPredictor:
    ...
    def predict(
        ...
        if len(predictions) == 1:
            predicted_jitter = float(predictions[0])
        else:
            predicted_jitter = float(np.mean(predictions))
        ...
        if return_confidence and hasattr(self.model, "estimators_"):
            try:
                ...
                if len(X_scaled) == 1:
                    ...
                    variability_estimate = float(np.mean((upper - lower) / 2.0))
        ...
        return JitterPrediction(
            ...
            variability_estimate=variability_estimate,
