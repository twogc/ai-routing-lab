...
class TestRouteOptimizationEnsemble:
    ...
    def mock_mab(self):
        ...
        mock.predict_sample.return_value = MagicMock(
            selected_route=1, 
            arm_statistics={1: {"mean": 0.5, "count": 10, "ucb": 0.6}}
        )
        mock.get_metrics.return_value = {}
        return mock
    ...
    def ensemble(self, mock_nn, mock_rf, mock_ql, mock_mab):
        with patch("models.routing.route_ensemble.NeuralNetworkRouteOptimizer", return_value=mock_nn), \
             patch("models.routing.route_ensemble.RandomForestRouteClassifier", return_value=mock_rf), \
